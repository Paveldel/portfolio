...
Wat hebben Moby Dick, Plato, een rode geit en LLaMA met elkaar gemeen? Op het eerste gezicht lijkt dat een vreemde combinatie, maar na de sessie DeepSeek Uncovered kijk ik helemaal anders naar de snelle opmars van de DeepSeek-modellen. De Tech & Meet werd op een diepgaande en duidelijke manier uitgelegd, terwijl het tegelijk ook entertainend bleef dankzij Dimitri’s sterke presentatie en enthousiasme.

DeepSeek is een LLM-model, vergelijkbaar met GPT-5, Gemini of de Claude-modellen, maar er is één groot verschil. Binnen de wereld van LLM’s blijven veel modellen gesloten en zijn hun weights niet publiek beschikbaar. Bij DeepSeek is dat anders: het model is open source, waardoor ontwikkelaars er zelf mee aan de slag kunnen.
...
Voor mensen die nog twijfels hebben over het gebruik van een Chinees model, bijvoorbeeld op vlak van security of privacy, bestaat er gelukkig een oplossing. Met Ollama kan je DeepSeek volledig offline en privé draaien op je eigen toestel, zonder afhankelijk te zijn van externe servers.
